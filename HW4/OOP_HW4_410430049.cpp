...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "TRA_ctrl_packet" /*&& !hi*/) { // the switch receives a packet from the controller
      ...
      // string msg = l3->getMsg(); // get the msg
        ...
        TRA_data_packet* dataPacket = dynamic_cast<TRA_data_packet*> (p);
        TRA_data_payload* payload = dynamic_cast<TRA_data_payload*>(p->getPayload());
        TRA_data_header* header = dynamic_cast<TRA_data_header*>(dataPacket->getHeader());
        
        unsigned int dst = header->getDstID();
        unsigned int packetsize = dataPacket->getSize();
        int num = table[dst].size();
        for(int i = 0; i < num; i ++){
            auto pp = table[dst][i];
            dataPacket->setSize(packetsize * pp.second);
            header->setNexID(pp.first);
            send_handler(dataPacket);
        }
...
vector<vector<int>> Create_DAG(vector<vector<int>> &graph, int node_num, int dst, vector<my_node> &n){
    ...
    vector<vector<bool>> graph2(node_num, vector<bool>(node_num, false));
    ...
    for (int i = 0; i < Dst_num; i++) {
		...
		for (int j = 0; j < node_num; j++) {
			...
			if (sum == 0) {
				...
				for (auto &k : DAG[j]) {
                    // cerr << "pay\n";
					n[j].save_flow[dst[i]].push_back({ k, 1 / sum });
				}
			}
            // for(auto &k:DAG[j]){
            //     n[j].save_flow[dst[i]].push_back({k, 1.0 / (double)DAG[j].size()});
            // }
		}
	}
}
...
void SDN_controller::recv_handler (packet *p){
    ...
    }else if(p->type() == "SDN_invoke_packet"){
        SDN_invoke_packet* invokePacket = dynamic_cast<SDN_invoke_packet*>(p);
        SDN_invoke_header* header = dynamic_cast<SDN_invoke_header*>(invokePacket->getHeader());
        SDN_invoke_payload* payload = dynamic_cast<SDN_invoke_payload*>(invokePacket->getPayload());

        vector<vector<double>> traffic_matrix = payload->getTrafficMatrix();
        // 0 dst_list
        // 1 sdn_list
        // 2 link_matrix
        // 3 pair_total
        vector<int> dst;
        int nodeNum = this->getNodeID();
        vector<my_node> n(nodeNum + 1);
        vector<vector<int>> graph(nodeNum);
        vector<vector<bool>> graph_matrix(nodeNum, vector<bool>(nodeNum, false));
        map<int, int> dst_list;
        for(int i = 0; i < traffic_matrix[0].size(); i ++){
            dst.push_back((int)traffic_matrix[0][i]);
            dst_list[(int)traffic_matrix[0][i]] = 1;
        }
        for(int i = 0; i < traffic_matrix[1].size(); i ++){
            n[(int)traffic_matrix[1][i]].change_type();
        }
        for(int i = 0; i < traffic_matrix[2].size(); i += 2){
            graph[(int)traffic_matrix[2][i]].push_back((int)traffic_matrix[2][i + 1]);
            graph[(int)traffic_matrix[2][i + 1]].push_back((int)traffic_matrix[2][i]);
            graph_matrix[(int)traffic_matrix[2][i]][(int)traffic_matrix[2][i + 1]] = true;
            graph_matrix[(int)traffic_matrix[2][i + 1]][(int)traffic_matrix[2][i]] = true;
        }
        map<int, vector<pair<int, int>>> save_source;
        for(int i = 0; i < traffic_matrix[3].size(); i += 4){
            int src = (int)traffic_matrix[3][i];
            int dst = (int)traffic_matrix[3][i + 1];
            int flowsize = (int)traffic_matrix[3][i + 2];
            int t = (int)traffic_matrix[3][i + 3];
            save_source[dst].push_back({src, flowsize});
        }
        func(dst, graph, nodeNum, graph_matrix, n, save_source);
        for(int i = 0; i < nodeNum; i++){
            if(n[i].is_SDN()){
                // node::id_to_node(nodeNum)->add_phy_neighbor(i);
                // node::id_to_node(i)->add_phy_neighbor(nodeNum);
                if(dst_list.find(i) != dst_list.end()) SDN_ctrl_packet_event(nodeNum, i, i, i, 1, event::getCurTime());
                for(auto t:n[i].save_flow){
                    for(int j = 0; j < t.second.size(); j++){
                        // cerr << i << ' ' << t.first << ' ' << t.second[j].first << ' ' << t.second[j].second << '\n';
                        SDN_ctrl_packet_event(nodeNum, i, t.first, t.second[j].first, t.second[j].second, event::getCurTime());
                    }
                }
            }
        }
...
int main(){
    ...
    int node_num, sdn_num, dst_num, link_num, pair_num, simtime, invoke_time;
    cin >> node_num >> sdn_num >> dst_num >> link_num >> pair_num >> simtime >> invoke_time;
    vector<double> sdn_list(sdn_num);
    vector<int> node_type(node_num, 0);
    for(int i = 0; i < sdn_num; i ++){
        cin >> sdn_list[i];
        node_type[sdn_list[i]] = 1;
    }
    vector<double> dst_list(dst_num);
    for(int i = 0; i < dst_num; i ++){
        cin >> dst_list[i];
    }
    ...
    node::node_generator::generate("SDN_controller", node_num);
    for(int i = 0; i < sdn_num; i ++){
        node::id_to_node(node_num)->add_phy_neighbor((int)sdn_list[i]);
        node::id_to_node((int)sdn_list[i])->add_phy_neighbor(node_num);
    }
    ...
    for(int i = 0; i < dst_num; i ++){
        int n, t;
        cin >> n >> t;
        TRA_ctrl_packet_event(n, t);
    }
    vector<double> link_matrix;
    for(int i = 0; i < link_num; i ++){
        int id, n1, n2;
        cin >> id >> n1 >> n2;
        link_matrix.push_back(n1);
        link_matrix.push_back(n2);
        node::id_to_node(n1)->add_phy_neighbor(n2);
        node::id_to_node(n2)->add_phy_neighbor(n1);
    }
    vector<double> pair_total;
    for(int i = 0; i < pair_num; i ++){
        int id, src, dst, flowsize, t;
        cin >> id >> src >> dst >> flowsize >> t;
        pair_total.push_back(src);
        pair_total.push_back(dst);
        pair_total.push_back(flowsize);
        pair_total.push_back(t);
        data_packet_event(src, dst, flowsize, t);
    }
    ...
    vector<vector<double>> traffic_matrix;
    traffic_matrix.resize(4);
    // 0 dst_list
    // 1 sdn_list
    // 2 link_matrix
    // 3 pair_total
    traffic_matrix[0] = dst_list;
    traffic_matrix[1] = sdn_list;
    traffic_matrix[2] = link_matrix;
    traffic_matrix[3] = pair_total;
    ...
    SDN_invoke_packet_event(node_num, traffic_matrix, invoke_time); 
    // 1st parameter: the controller id
    // 2nd parameter: time (optional)
    // 3rd parameter: msg for debug information (optional)
    ...
    // data_packet_event(4, 0, 10.5, 200); // data_packet is type TRA_data_packet

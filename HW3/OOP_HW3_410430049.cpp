...
vector<vector<int>> Create_DAG(vector<vector<int>> &graph, int node_num, int dst, vector<flow_node> &n){
...
map<int, int> level_graph(int dst, vector<vector<int>> &graph){
    map<int, int> level;
    queue<int> save;
    save.push(dst);
    level[dst] = 0;
...
int max_flow(int s, int t, int flow, vector<vector<Edge>> &G, vector<flow_node> &n) {
	int max_flow = 0;
...
void save_node_packet(vector<flow_node> &n, vector<vector<int>> &DAG, vector<vector<int>> &adj_m, vector<pair<int, int>> &s){
...
bool judge_max_flow(int packet, vector<flow_node> &n, vector<pair<int, int>> &source, vector<vector<int>> &DAG, int dst, vector<vector<Edge>> &G) {
...
vector<vector<int>> Create_DAG_graph(vector<vector<int>> &graph, int node_num, int dst, vector<flow_node> &n){
    vector<vector<int>> DAG(node_num);
    queue<int> save;
    save.push(dst);
    vector<bool> visit(node_num, false);
    vector<vector<bool>> check(node_num, vector<bool>(node_num, false));
    /// visit[dst] = true;
    auto level_map = level_graph(dst, graph);
...
void max_link_load_ctrl(vector<int> &DST_List, vector<vector<int>> &graph, vector<flow_node> &n, map<int, vector<pair<int, int>>> &save_dst){
    int Dst_num = DST_List.size();
    int node_num = graph.size();
    for(int i = 0; i < Dst_num; i++){
        auto DAG = Create_DAG(graph, node_num, DST_List[i], n);
        vector<vector<Edge>> G;
		int flow_size;
		int left = 0, right = 2000;
        vector<int> table(3000, -1);
		G.resize(node_num + 1);
		while(left <= right){
			int mid = (left + right) / 2;
			if(table[mid - 1] == -1){
				G.resize(node_num + 1);
				table[mid - 1] = judge_max_flow(mid - 1, n, save_dst[DST_List[i]], DAG, DST_List[i], G) ? 1 : 0;
				G.clear();
			}
			if(table[mid] == -1){
				G.resize(node_num + 1);
				table[mid] = judge_max_flow(mid, n, save_dst[DST_List[i]], DAG, DST_List[i], G) ? 1 : 0;
				G.clear();
			}
		...
		G.clear();
		G.resize(node_num + 1);
		judge_max_flow(flow_size, n, save_dst[DST_List[i]], DAG, DST_List[i], G);
		vector<vector<int>> adj_m(node_num, vector<int>(node_num, 0));
		for (int j = 0; j < node_num; j++) {
			for (int k = 0; k < G[j].size(); k++) {
				if (G[j][k].to >= node_num || G[j][k].from >= node_num) continue;
				adj_m[G[j][k].from][G[j][k].to] = flow_size - G[j][k].cap;
			}
		}
		save_node_packet(n, DAG, adj_m, save_dst[DST_List[i]]);
		for (int j = 0; j < node_num; j++) {
			double sum = (double)n[j].packet;
			if (sum == 0) {
				sum = DAG[j].size();
				for (auto &k : DAG[j]) {
					n[j].save_flow[DST_List[i]].push_back({ k, 1.0 / sum });
				}
			}
			else {
				for (auto &k : DAG[j]) {
					n[j].save_flow[DST_List[i]].push_back({ k, adj_m[j][k] / sum });
				}
			}
		}
    }
...
int main()
{
    ...
    max_link_load_ctrl(dl, graph, n, save_source);
    for(int i = 0; i < node_num; i++){
        if(n[i].is_SDN()){
